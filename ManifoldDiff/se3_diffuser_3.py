...
class SE3Diffusion_3:
    ...
    def forward_process(self, x1, x2, t):
        ...
        return (x1_t, epsilon1), (x1_t, epsilon2)
    ...
    def compute_loss(self, x1, x2, t):
        ...
        if self.score_model.name == "Unet":
            ...
            x1_t = x1_t.transpose(1,2)
            x2_t = x2_t.transpose(1,2)
        x0 = extract(self.x0_param1, t, x1_t.shape) * (x1_t - extract(self.x0_param2, t, x1_t.shape) * predicted_score1)
        
        # Loss for each stream
        loss1 = F.l1_loss(predicted_score1, epsilon1)
        loss_origin = F.l1_loss(x0, x1)
        loss2 = F.l1_loss(predicted_score2, epsilon2)

        # Average the two losses
        return loss1 + loss_origin

    def sample(self, shape, device, num_steps=30):
        """
        ...
        with torch.no_grad():
            ...
            for t in range(num_steps-1, -1, -1):
                ...
                mean = extract(self.mean_param1, t, x1_t.shape) * x1_t + extract(self.mean_param2, t, x1_t.shape) * x0
                sigma = extract(self.sigma, t, x1_t.shape)
        ...
        for epoch in range(epochs):
            ...
            with tqdm(data_loader, desc=f"Epoch {epoch + 1}/{epochs}", unit="batch") as pbar:
                for i, batch in enumerate(pbar):
                    ...
                    loss = self.compute_loss(translations * self.trans_scale, rotations, t)

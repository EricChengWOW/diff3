...
class R3Conf:
  def __init__(self, coordinate_scaling, num_timesteps, min_b, max_b) -> None:
...
class R3Diffuser:
    ...
    def forward(self, x_t_1: np.ndarray, t: float, num_t: int):
        ...
        z_t_1 = torch.tensor(np.random.normal(size=x_t_1.shape)).to(x_t_1.device).to(x_t_1.dtype)
    ...
    def forward_marginal(self, x_0: np.ndarray, t: float):
        ...
        #     raise ValueError(f'{t} must be a scalar.')
        x_0 = self._scale(x_0)
        # x_t = torch.random.normal(
        #     loc=torch.exp(-1/2*self.marginal_b_t(t)) * x_0,
        #     scale=torch.sqrt(1 - torch.exp(-self.marginal_b_t(t)))
        # )
        x_t = torch.exp(-1/2*self.marginal_b_t(t)) * x_0 + \
    ...
    def score(self, x_t, x_0, t, use_torch=True, scale=True):
        if use_torch:
            exp_fn = torch.exp
        else:
            exp_fn = np.exp

...
class TorchGMM(nn.Module):
    """PyTorch implementation of a Gaussian Mixture Model."""
    def __init__(self, n_components, n_features):
        ...
        self.covs = nn.Parameter(torch.ones(n_components, n_features))  # Diagonal variances
        self.weights = nn.Parameter(torch.ones(n_components) / n_components)
        self.reg_covar = 1e-5

    def ensure_positive_semidefinite(self, matrix):
        """Ensures that the covariance matrix is positive semi-definite (PSD)."""
        # Clamp extreme values to prevent NaN issues
        matrix = torch.clamp(matrix, min=-1e5, max=1e5)

        # Add small regularization term to the diagonal
        matrix += torch.eye(matrix.shape[-1], device=matrix.device) * self.reg_covar
        
        try:
            # Check if the matrix is positive semi-definite (PSD)
            torch.linalg.cholesky(matrix)
        except RuntimeError:
            print("Warning: Covariance matrix is not PSD, adding stronger regularization!")
            matrix += torch.eye(matrix.shape[-1], device=matrix.device) * self.reg_covar * 10
        
        return matrix
        
    def forward(self, x):
        """Computes log-likelihood of data given the mixture model."""
        batch_size = x.shape[0]
        likelihoods = []

        for i in range(self.n_components):
            # cov_matrix = self.covs[i] + torch.eye(self.n_features, device=self.covs.device) * self.reg_covar  # Ensure positivity
            cov_matrix = self.ensure_positive_semidefinite(self.covs[i])
            mvn = MultivariateNormal(self.means[i], cov_matrix)
            print(torch.mean(self.means[i]), torch.mean(cov_matrix))
            likelihoods.append(mvn.log_prob(x))  # Convert log-prob to probability
        
        likelihoods = torch.stack(likelihoods, dim=-1)  # Shape: (batch_size, n_components)
        weighted_likelihoods = likelihoods * torch.softmax(self.weights, dim=0)  # Apply weights
        return torch.sum(weighted_likelihoods, dim=-1)  # Summed over components

    def get_log_likelihood(self, x):
        """Computes negative log-likelihood as the loss."""
        log_likelihood = torch.log(self.forward(x) + 1e-8)  # Avoid log(0)
...
class GaussianMixture(torch.nn.Module):
    ...
    """
    def __init__(self, n_components, n_features, covariance_type="full", eps=1.e-6, init_params="kmeans", mu_init=None, var_init=None, device='cpu'):
        """
    ...
    def _init_params(self):
        if self.mu_init is not None:
            ...
            # (1, k, d)
            self.mu = torch.nn.Parameter(self.mu_init.to(self.device), requires_grad=False)
        else:
            self.mu = torch.nn.Parameter(torch.randn(1, self.n_components, self.n_features, device=self.device), requires_grad=False)
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(self.var_init.to(self.device), requires_grad=False)
            else:
                self.var = torch.nn.Parameter(torch.ones(1, self.n_components, self.n_features, device=self.device), requires_grad=False)
            ...
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(
                    torch.eye(self.n_features, device=self.device).reshape(1, 1, self.n_features, self.n_features).repeat(1, self.n_components, 1, 1),
                    requires_grad=False
                )

        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.ones(1, self.n_components, 1, dtype=torch.float32, device=self.device) / self.n_components, requires_grad=False)
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            ...
            # Ensure `var` is positive definite before Cholesky decomposition
            eps = self.eps  # Start with small regularization

            reg_var = var + torch.eye(var.shape[-1], device=var.device) * eps  # Regularize diagonal
            chol_factor = torch.linalg.cholesky(reg_var)  # Cholesky decomposition
            # for i in range(10):  # Try increasing `eps` gradually if needed
            #     try:
            #         reg_var = var + torch.eye(var.shape[-1], device=var.device) * eps  # Regularize diagonal
            #         chol_factor = torch.linalg.cholesky(reg_var)  # Cholesky decomposition
            #         break  # Success: exit loop
            #     except torch._C._LinAlgError:
            #         eps *= 10  # Increase regularization factor

            if eps > 1e-2:
                print(f"Warning: Large regularization (eps={eps}) needed for Cholesky!")

            # Compute inverse from Cholesky decomposition
            precision = torch.cholesky_inverse(chol_factor)

            # 🔹 **Ensure precision is still PSD** by clamping eigenvalues
            eigvals_prec, eigvecs_prec = torch.linalg.eigh(precision)
            eigvals_prec = torch.clamp(eigvals_prec, min=1e-6)  # Ensure all eigenvalues are positive
            precision = eigvecs_prec @ torch.diag_embed(eigvals_prec) @ eigvecs_prec.transpose(-2, -1)


            # # Now it's safe to invert
            # precision = torch.inverse(var_psd)  # var_psd is guaranteed to be invertible
            # # Ensure `precision` is also PSD
            # eigvals_prec, eigvecs_prec = torch.linalg.eigh(precision)
            # eigvals_prec = torch.clamp(eigvals_prec, min=self.eps)  # Ensure positive eigenvalues
            # precision_psd = eigvecs_prec @ torch.diag_embed(eigvals_prec) @ eigvecs_prec.transpose(-2, -1)
    ...
    def _m_step(self, x, log_resp):
        ...
        if self.covariance_type == "full":
            eps = (torch.eye(self.n_features, device=self.device) * self.eps).to(x.device)
